...
def enable(
    ...
    command: str = typer.Option("php artisan schedule:work", "--command", help="Scheduler command."),
...
def reconcile(
    ...
    app_ctx.emit_output(
        "ok",
        f"Scheduler reconcile plan prepared for {domain}",
        domain=domain,
        apply=apply,
        dry_run=app_ctx.dry_run,
    )
    ...
    try:
        ...
        app_ctx.emit_output("error", str(exc))
        raise typer.Exit(code=2) from exc
    app_ctx.emit_output("ok", f"Scheduler reconciled for {domain}", result=result)
